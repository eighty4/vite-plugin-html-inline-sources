...
export async function runViteBuild(opts) {
    ...
    if (!opts.files || !Object.keys(opts.files).length)
        throw new Error('opts.files is required')
    ...
    await symlink(
        process.cwd(),
        joinPath(root, 'node_modules/vite-plugin-html-inline-sources'),
    )
    await symlink(
        joinPath(process.cwd(), 'node_modules/vite'),
        joinPath(root, 'node_modules/vite'),
    )
    ...
    return new Promise((res, rej) => {
        const viteProcess = child_process.spawn(
            'node',
            [joinPath(process.cwd(), 'node_modules/vite/bin/vite.js'), 'build'],
            { cwd: root },
        )
        let output = ''
