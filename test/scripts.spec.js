...
new Foo("0").isAsdfy();
    ...
    describe('errors', () => {
        ...
        it('for network src', async () => {
            ...
            assert.ok(result.error, result.output)
            assert.ok(result.output.includes('<script vite-inline> muse use a relative filesystem src path (network paths like https://cdn/index.js are unsupported)'), result.output)
            await rm(result.root, {recursive: true})
        })
